...
const GoogleAuth = (() => {
  ...
  async function exchange(mode, code, state) {
    const endpoint = mode === 'link' ? '/api/google/link' : '/api/google/login';
    const res = await fetch(endpoint, {
      method: 'POST',
  ...
  async function launch(mode) {
    ...
    const endpoint = mode === 'link' ? '/api/google/link-url' : '/api/google/url';
    if (!el) return;
  ...
  // troca o code e conclui o login (ou o vínculo, quando no perfil).
  async function handlePageParams(mode) {
    ...
    try {
      await exchange(mode, code, state);
      ...
      window.location.replace(`/login?error=google${reason}`);
      return true;
    }
  }
  ...
  function init() {
    ...
    if (hasCode) {
      ...
      // na página certa (login/registro → /api/google/login; perfil → /api/google/link).
